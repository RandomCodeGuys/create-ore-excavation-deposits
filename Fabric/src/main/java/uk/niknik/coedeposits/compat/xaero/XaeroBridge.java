...
public final class XaeroBridge {
     ...
     * substituting placeholders. Literal text passes through verbatim (§ colour
     * codes render); unknown {@code %token%}s are left as-is.
     */
    ...
    private static MutableComponent placeholder(String token, DepositDiscoveryPayload p) {
        return switch (token) {
            ...
            case "player" -> Component.literal(p.player()).withStyle(ChatFormatting.GREEN);
