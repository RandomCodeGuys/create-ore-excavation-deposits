 ...
 *
 * <p><b>Fabric 1.20.1 line.</b> This is a verbatim copy of the platform-shared
 * {@code network/DepositSnapshot} from the Forge module — it has no
 * loader-specific imports (only vanilla MC + the cross-loader COE API). It lives
 * in {@code Fabric/src/main} because the Fabric build excludes
 * {@code **&#47;network/**} from the platform-shared srcDir (that package's
 * {@link CoedepositsNetwork} / payload classes are Forge-only). Keep this file
 * byte-compatible on the wire with the Forge version: any encode/decode reorder
 * MUST mirror the other loader's copy.
 *
 * <p>OreData read through {@link OreDataCapability}; {@code OreData.getRecipe}
 * returns a bare {@link VeinRecipe} (no {@code RecipeHolder});
 * {@code RecipeManager.getRecipes()} yields bare {@link Recipe}.
 *
 * <p>{@code dimension} carries the source level's id so the client renderer can
 * skip snapshots from other dimensions. {@code revealModeOrdinal} mirrors the
 * server-side effective reveal mode; {@code proximityBlocks} is the ON_PROXIMITY
 * radius (0 when N/A). {@code recipeIds}/{@code recipeWeights} are the type's
 * full weighted-recipe pool; {@code chunkRecipeIndex} is parallel to
 * {@code packedChunks} (each entry indexes {@code recipeIds}, or {@code -1} for a
 * filler).
 */
public record DepositSnapshot(
    ...
    private static long computeRemaining(ServerLevel lvl,
        ...
        if (vr == null) {
            // -4: recipe id set but unresolvable (misconfig) vs -1 (genuinely cleared).
