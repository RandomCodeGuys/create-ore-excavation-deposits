 ...
 * the value to the {@link ForgeConfigSpec} AND flushes to disk immediately, so
 * persistence does not depend on YACL's separate save callback running after apply.
 * {@link #toggle} is the ONE factory we keep — a boolean is a trivial 3-arg
 * declaration, so a helper is justified there; every option with ranges/scales/enums
 * is written out explicitly in its group file.
 */
public final class Bindings {
    private Bindings() {}

    /** Write a value to its config entry and flush to disk; diagnostic log when lifecycle logging is on. */
    public static <T> void persist(ForgeConfigSpec.ConfigValue<T> v, T val, String name) {
    ...
    public static Option<Boolean> toggle(String name, String desc, ForgeConfigSpec.BooleanValue v) {
