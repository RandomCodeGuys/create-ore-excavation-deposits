...
public final class WorldMapDepositRenderer {
    ...
    public static void render(
        ...
        if (hovered != null) {
            // Hand the hovered deposit to the share keybind (the Xaero overlay's
            // key handler reads hoveredDepositId on the share-key press).
    ...
    private static List<Component> buildTooltip(DepositSnapshot d, long hoveredChunkLong) {
        ...
        if (!CoedepositsFabricClient.SHARE_DEPOSIT.isUnbound()) {
            out.add(Component.literal("[")
                    .append(CoedepositsFabricClient.SHARE_DEPOSIT.getTranslatedKeyMessage())
