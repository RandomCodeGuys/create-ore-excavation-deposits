 ...
 * YACL option whose getter/setter read/write the same {@link ForgeConfigSpec}
 ...
 * {@code ForgeConfigSpec} types being bound; with Forge Config API Port the same
...
public final class YaclConfigScreen {
    ...
    // ── option helpers (bind each YACL Option straight to its ForgeConfigSpec value)

    /**
     * Binding setter shared by every option helper. Writes the value to the
     * ForgeConfigSpec AND flushes to disk immediately — so persistence no longer
     ...
     */
    private static <T> void persist(ForgeConfigSpec.ConfigValue<T> v, T val, String name) {
    ...
    private static Option<Double> dbl(String name, String desc, ForgeConfigSpec.DoubleValue v,
    ...
    /**
     * Bind an Integer YACL field to a {@link ForgeConfigSpec.DoubleValue} through a
     ...
     */
    private static Option<Integer> scaledDbl(String name, String desc, ForgeConfigSpec.DoubleValue v,
    ...
    private static Option<Integer> intOpt(String name, String desc, ForgeConfigSpec.IntValue v, int min, int max) {
        return Option.<Integer>createBuilder()
                .name(Component.literal(name))
                .description(OptionDescription.of(Component.literal(desc)))
                .binding(v.getDefault(), v::get, val -> persist(v, val, name))
    ...
    private static Option<Boolean> bool(String name, String desc, ForgeConfigSpec.BooleanValue v) {
    ...
    private static <E extends Enum<E> & StringRepresentable> Option<E> enumOpt(
            String name, String desc, ForgeConfigSpec.EnumValue<E> v, Class<E> cls) {
     ...
     * (dropdown per entry + add/remove), writing back to the ForgeConfigSpec list.
