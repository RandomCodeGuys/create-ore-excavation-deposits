...
public final class CoedepositsClientEvents {
    ...
    /**
     * Share the deposit hovered on the world map to chat (clickable [+ Add to map]
     * offer for everyone). Default unbound, UNIVERSAL context so it fires while the
     * Xaero map screen is open — same pattern as {@link #TOGGLE_OVERLAY}.
     */
    ...
    public static final class Game {
         ...
         * KeyMappings (consumeClick stays 0), so the map-screen share key is
         * caught at the screen level: match the raw key against the binding and
         * act when a deposit is hovered.
         */
        @SubscribeEvent
        public static void onScreenKeyPressed(net.minecraftforge.client.event.ScreenEvent.KeyPressed.Pre event) {
            ...
            uk.niknik.coedeposits.network.CoedepositsNetwork.CHANNEL.sendToServer(
