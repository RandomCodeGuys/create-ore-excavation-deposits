 ...
 * KubeJS converts the JS object to a {@link JsonObject} automatically. A scripted
 * type must reference a real vein recipe — inline {@code vein}/{@code drilling}/
 * {@code fluid} synthesis only runs for the on-disk config overlay (build the
 * recipe with the host mod's own KubeJS API if you need one). Re-runs overwrite
 * by id; {@link #remove} drops one.
 */
